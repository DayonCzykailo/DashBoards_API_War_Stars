...
async function atualizaGraficos() {
    await filtra().then(result => result.results === undefined ? result : result.results).then(datapoints => {
        console.log(datapoints);
...
async function carregaMenu() {
    var nomes = await fetchedData.then(datapoints => {
    ...
    nomes.forEach(function (nome) {
        ...
        //Criando elementos
        var novaTag = document.createElement("h3");
        novaTag.appendChild(document.createTextNode(nome));
        novaTag.onclick = () => {
        ...
        var corpo = document.querySelector(".menuNave");
        corpo.appendChild(novaTag);
